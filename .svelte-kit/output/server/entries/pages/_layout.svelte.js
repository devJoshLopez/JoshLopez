...
const Navigation = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<header class="bg-gray-900 text-white border-b-4 border-pink-600 p-4 text-center fixed top-0 w-full z-20" data-svelte-h="svelte-16vep9k"><h1 class="text-4xl">Josh Lopez</h1> <nav class="mt-4"><a href="${escape(base, true) + "/"}" class="text-pink-600 hover:text-teal-400 mx-4 glow-on-hover">Home</a> <a href="${escape(base, true) + "/videos"}" class="text-pink-600 hover:text-teal-400 mx-4 glow-on-hover">Videos</a> <a href="${escape(base, true) + "/writings"}" class="text-pink-600 hover:text-teal-400 mx-4 glow-on-hover">Writings</a> <a href="${escape(base, true) + "/projects"}" class="text-pink-600 hover:text-teal-400 mx-4 glow-on-hover">Projects</a> <a href="${escape(base, true) + "/connect"}" class="text-pink-600 hover:text-teal-400 mx-4 glow-on-hover">Connect</a></nav></header>`;
});
